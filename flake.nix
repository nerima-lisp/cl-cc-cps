{
  ...
  outputs =
    ...
    let
      ...
      # CI builds and tests only x86_64-linux, matching cl-cc-bootstrap and
      # cl-cc-type (the org's newest siblings as of this extraction).
      # `mkPackageFlake` generates every per-system output from this one
      # list — packages, checks, apps and devShells alike — so this is also
      # what `nix develop`/`nix build` resolve against.
      systems = [
        "x86_64-linux"
      ];
